...
<div class="container">
        ...
        {{-- Mahasiswa (list with safety) --}}
        <h5 class="mt-4">Data Mahasiswa & APD</h5>
        <p class="text-muted">Gunakan tombol Hapus untuk mengeluarkan mahasiswa dari JSA. Pilih nilai APD untuk masing-masing mahasiswa.</p>

        {{-- Selected mahasiswa (pre-populated from $jsa->mahasiswas) --}}
        <ul id="selected-mahasiswa" class="list-group mb-3">
            @php
                // mapping apd by id_mhs untuk mudah akses
                $apdMap = [];
                foreach($jsa->apds as $apd) {
                    $apdMap[$apd->id_mhs] = $apd;
                }
            @endphp

            @foreach($jsa->mahasiswas as $mhs)
                @php $apd = $apdMap[$mhs->id] ?? null; @endphp
                <li class="list-group-item list-item flex-column align-items-start" data-id="{{ $mhs->id }}">
                    {{-- Hidden mahasiswa id (mahasiswas[]) --}}
                    <input type="hidden" name="mahasiswas[]" value="{{ $mhs->id }}">
                    {{-- Also include apds[] marker (used by some store implementations) --}}
                    <input type="hidden" name="apds[]" value="{{ $mhs->id }}">

                    <div class="d-flex w-100 justify-content-between align-items-center mb-2">
                        <div class="fw-semibold">{{ $mhs->nim }} - {{ $mhs->nama }}</div>
                        <button type="button" class="btn btn-sm btn-danger btn-remove-mhs">Hapus</button>
                    </div>

                    {{-- Safety inputs: we output both "safety[<id>][key]" and apds[index][...] formats
                         so controller store/update yang berbeda bisa menerima --}}
                    @php
                        $shelmet = old("safety.{$mhs->id}.shelmet", $apd->apd_shelmet ?? 'Tidak');
                        $sgloves = old("safety.{$mhs->id}.sgloves", $apd->apd_sgloves ?? 'Tidak');
                        $shoes = old("safety.{$mhs->id}.shoes", $apd->apd_shoes ?? 'Tidak');
                        $sglasses = old("safety.{$mhs->id}.sglasses", $apd->apd_sglasses ?? 'Tidak');
                        $svest = old("safety.{$mhs->id}.svest", $apd->apd_svest ?? 'Tidak');
                        $earplug = old("safety.{$mhs->id}.earplug", $apd->apd_earplug ?? 'Tidak');
                        $fmask = old("safety.{$mhs->id}.fmask", $apd->apd_fmask ?? 'Tidak');

                        // For the alternate apds[][] format we will build inputs via JS when form submits,
                        // but we also render one set now with index = the mhs id to be safe:
                    @endphp

                    <div class="safety-row mt-1">
                        <div class="row gx-2">
                            <div class="col-12 col-md-6 mb-1 d-flex align-items-center">
                                <label class="mb-0">Safety Helmet</label>
                                <select name="safety[{{ $mhs->id }}][shelmet]" class="form-select form-select-sm ms-2 w-auto">
                                    <option value="Ada" {{ $shelmet === 'Ada' ? 'selected' : '' }}>Ada</option>
                                    <option value="Tidak" {{ $shelmet === 'Tidak' ? 'selected' : '' }}>Tidak</option>
                                </select>
                            </div>
                            <div class="col-12 col-md-6 mb-1 d-flex align-items-center">
                                <label class="mb-0">Safety Gloves</label>
                                <select name="safety[{{ $mhs->id }}][sgloves]" class="form-select form-select-sm ms-2 w-auto">
                                    <option value="Ada" {{ $sgloves === 'Ada' ? 'selected' : '' }}>Ada</option>
                                    <option value="Tidak" {{ $sgloves === 'Tidak' ? 'selected' : '' }}>Tidak</option>
                                </select>
                            </div>

                            <div class="col-12 col-md-6 mb-1 d-flex align-items-center">
                                <label class="mb-0">Safety Shoes</label>
                                <select name="safety[{{ $mhs->id }}][shoes]" class="form-select form-select-sm ms-2 w-auto">
                                    <option value="Ada" {{ $shoes === 'Ada' ? 'selected' : '' }}>Ada</option>
                                    <option value="Tidak" {{ $shoes === 'Tidak' ? 'selected' : '' }}>Tidak</option>
                                </select>
                            </div>
                            <div class="col-12 col-md-6 mb-1 d-flex align-items-center">
                                <label class="mb-0">Safety Glasses</label>
                                <select name="safety[{{ $mhs->id }}][sglasses]" class="form-select form-select-sm ms-2 w-auto">
                                    <option value="Ada" {{ $sglasses === 'Ada' ? 'selected' : '' }}>Ada</option>
                                    <option value="Tidak" {{ $sglasses === 'Tidak' ? 'selected' : '' }}>Tidak</option>
                                </select>
                            </div>

                            <div class="col-12 col-md-6 mb-1 d-flex align-items-center">
                                <label class="mb-0">Safety Vest</label>
                                <select name="safety[{{ $mhs->id }}][svest]" class="form-select form-select-sm ms-2 w-auto">
                                    <option value="Ada" {{ $svest === 'Ada' ? 'selected' : '' }}>Ada</option>
                                    <option value="Tidak" {{ $svest === 'Tidak' ? 'selected' : '' }}>Tidak</option>
                                </select>
                            </div>
                            <div class="col-12 col-md-6 mb-1 d-flex align-items-center">
                                <label class="mb-0">Earplug</label>
                                <select name="safety[{{ $mhs->id }}][earplug]" class="form-select form-select-sm ms-2 w-auto">
                                    <option value="Ada" {{ $earplug === 'Ada' ? 'selected' : '' }}>Ada</option>
                                    <option value="Tidak" {{ $earplug === 'Tidak' ? 'selected' : '' }}>Tidak</option>
                                </select>
                            </div>

                            <div class="col-12 mb-1 d-flex align-items-center">
                                <label class="mb-0">Face Mask / Masker</label>
                                <select name="safety[{{ $mhs->id }}][fmask]" class="form-select form-select-sm ms-2 w-auto">
                                    <option value="Ada" {{ $fmask === 'Ada' ? 'selected' : '' }}>Ada</option>
                                    <option value="Tidak" {{ $fmask === 'Tidak' ? 'selected' : '' }}>Tidak</option>
                                </select>
                            </div>
                        </div>
                    </div>

                    {{-- Also include hidden inputs for apds[index][id_mhs] and apds[index][apd_xxx]
                         We'll create index based on mahasiswa id for deterministic names.
                         This supports controller variants that expect apds as array of arrays. --}}
                    <input type="hidden" name="apds[{{ $mhs->id }}][id_mhs]" value="{{ $mhs->id }}">
                    <input type="hidden" name="apds[{{ $mhs->id }}][apd_shelmet]" value="{{ $shelmet }}">
                    <input type="hidden" name="apds[{{ $mhs->id }}][apd_sgloves]" value="{{ $sgloves }}">
                    <input type="hidden" name="apds[{{ $mhs->id }}][apd_shoes]" value="{{ $shoes }}">
                    <input type="hidden" name="apds[{{ $mhs->id }}][apd_sglasses]" value="{{ $sglasses }}">
                    <input type="hidden" name="apds[{{ $mhs->id }}][apd_svest]" value="{{ $svest }}">
                    <input type="hidden" name="apds[{{ $mhs->id }}][apd_earplug]" value="{{ $earplug }}">
                    <input type="hidden" name="apds[{{ $mhs->id }}][apd_fmask]" value="{{ $fmask }}">
                </li>
            @endforeach
        </ul>

        {{-- Dosen (list) --}}
        <h5 class="mt-4">Data Dosen</h5>
        <ul id="selected-dosen" class="list-group mb-3">
            @foreach($jsa->dosens as $d)
                <li class="list-group-item d-flex justify-content-between align-items-center" data-id="{{ $d->id }}">
                    <input type="hidden" name="dosens[]" value="{{ $d->id }}">
                    <div>{{ $d->nip }} - {{ $d->nama }}</div>
                    <div class="text-muted">Catatan: {{ $d->pivot->catatan_dsn ?? ( ($d->catatan_dsn ?? '') ?: 'Menunggu Catatan Dosen') }}</div>
                </li>
            @endforeach
        </ul>

        {{-- Work Steps --}}
        <h5 class="mt-4">Urutan Pekerjaan & Analisis Bahaya</h5>
        @if($jsa->workSteps && $jsa->workSteps->count() > 0)
                    ...
                    <tbody>
                        @foreach($jsa->workSteps as $step)
                            <tr>
                                ...
                                <td>{{ $step->potensi_bahaya }}</td>
                                <td>{{ $step->upaya_pengendalian }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
        ...
        @else
            <p class="text-muted">Tidak ada data urutan pekerjaan.</p>
        @endif

        {{-- Inspections --}}
        <h5 class="mt-4">Area Inspeksi</h5>
        @if($jsa->inspections && $jsa->inspections->count() > 0)
            @php
                $inspectionsByArea = $jsa->inspections->groupBy('area_inspeksi');
            @endphp
            @foreach($inspectionsByArea as $areaName => $inspections)
                <div class="card mb-3">
                    <div class="card-header">
                        <h6 class="mb-0">{{ $areaName }}</h6>
                    </div>
                    <div class="card-body">
                                ...
                                <tbody>
                                    @foreach($inspections as $inspection)
                                        <tr>
                                            ...
                                            <td>
                                                <span class="badge {{ $inspection->ok_ng === 'OK' ? 'bg-success' : 'bg-danger' }}">
                                                    {{ $inspection->ok_ng }}
                                                </span>
                                            </td>
                                            <td>{{ $inspection->tindakan_korektif }}</td>
                                            <td>{{ $inspection->tanggal_selesai ? $inspection->tanggal_selesai->format('d/m/Y') : '-' }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
            ...
            @endforeach
        @else
            <p class="text-muted">Tidak ada data area inspeksi.</p>
        @endif
